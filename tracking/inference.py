...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Update allPossible with newPos from distribution over new positions for the ghost
        for oldPos in self.beliefs:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos in newPosDist:
                # All possible positions for ghost is based on newPosDist and beliefs from oldPos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Traverse through all states in legalPositions to fill in list of all particles
    ...
    def observe(self, observation, gameState):
        ...
        for j in self.particles:
            # 1st Special Case - i.e. when a ghost is captured by Pacman
            if noisyDistance is None:
                allPossible[self.getJailPosition()] = 1.0
                break
            trueDist = util.manhattanDistance(j, pacmanPosition)
            if emissionModel[trueDist] > 0:
                allPossible[j] += emissionModel[trueDist]

        # Normalize allPossible
        allPossible.normalize()
        self.beliefs = allPossible

        # 2nd Special Case - i.e. when all particles receive 0 weight, they are recreated from prior distribution
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in beliefs:
            ...
            for newPos in newPosDist:
                # All possible positions for ghost is based on newPosDist and beliefs from oldPos
                allPossible[newPos] += (newPosDist[newPos] * beliefs[oldPos])

        allPossible.normalize()

        for j in range(len(self.particles)):
            ghostPos = util.sample(allPossible)
            self.particles[j] = ghostPos
