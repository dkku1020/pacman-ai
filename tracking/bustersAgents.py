...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Traverse livingGhostPositionDistributions to find ghostPos with the maxProbability
        # i.e. find the ghostPos with the highest probability based on the distribution
        for ghosts in livingGhostPositionDistributions:
            for moves in ghosts:
                if ghosts[moves] > maxProb:
                    maxProb = ghosts[moves]
                    chosenMove = moves
                    ghostPos[i] = (maxProb, chosenMove)
                else:
                    i += 1

        # From all the legal actions, move to where the distance between successorPosition and the chosen move
        # based on ghost positions is the smallest
        chosenAction = 0
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successorPosition, chosenMove) < minDist:
                minDist = self.distancer.getDistance(successorPosition, chosenMove)
                chosenAction = action
        return chosenAction